...
//  GTA
//
//  Created by Kostiantyn Dzetsiuk on 21.05.2021.
//
...
class Switch: UIControl {
    ...
    var borderWidt: CGFloat = 0 {
        didSet {
            trackLayer.borderWidth = borderWidt
    ...
    var thumbRadiusPadding: CGFloat = 4 {
        didSet {
            layoutThumbLayer(for: layer.bounds)
        }
    }
    
    var onTintColor: UIColor = UIColor(hex: 0xCC0000) {//UIColor(red: 0, green: 122/255, blue: 1, alpha: 1) {
        didSet {
            trackLayer.backgroundColor = getBackgroundColor()
        }
    }

    var offTintColor: UIColor = UIColor(hex: 0xAEAEB2) {
        didSet {
            trackLayer.backgroundColor = getBackgroundColor()
    ...
    func controlDidLoad() {
        ...
        innerLayer.backgroundColor = UIColor(red: 199.0 / 255.0, green: 199.0 / 255.0, blue: 203.0 / 255.0, alpha: 1.0).cgColor
    ...
    func layoutInnerLayer(for bounds: CGRect) {
        let inset = borderWidt + trackTopBottomPadding
    ...
    func layoutOffContentLayer(for bounds: CGRect) {
        ...
        let y = isTextLayer
            ...
            : borderWidt
        let leading = contentLeadingTrailingPadding + borderWidt
    ...
    func stateDidChange() {
        trackLayer.backgroundColor = getBackgroundColor()
        trackLayer.borderWidth = isOn ? 0 : borderWidt
    }
...
//        let height = bounds.height - 2 * (borderWidt + thumbRadiusPadding)
//        let width = (isTouchDown && isStretchEnable) ? height * 1.2 : height
    ...
    final func getContentLayerSize(for layer: CALayer?) -> CGSize {
        let inset = 2 * (borderWidt + trackTopBottomPadding)
